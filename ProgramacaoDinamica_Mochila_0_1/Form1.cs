...
namespace ProgramacaoDinamica_Mochila_0_1
{
    public partial class Form1 : Form
    {
        ...
        {
            MessageBox.Show("O aplicativo ProgramacaoDinamica_Mochila_0_1 é baseado exercício 16.2-2 " +
                           "disponível do livro Algoritmos de T. H. Cormen e foi desenvolvido " +
                           "por Leandro Poloni Dantas como atividade complementar e voluntária " +
                           "da displina de Algoritmos Computacionais, cursada durante seu doutorado na área de " +
                           "Dsipositivos Eletrônicos Integrados pelo Centro universitário da FEI.\n(Dezembro de 2014)",
                           "Sobre o Aplicativo",
        ...
        private void bExibir_Click(object sender, EventArgs e)
        {
            //Se selecionou algoritmo Mochila 0-1 (força bruta)
            if (rbMochilaFB.Checked)
            {
                Form2 formfilho = new Form2("Mochila 0-1 (força bruta)",
@"");
                formfilho.Show();
            }
            //Se selecionou algoritmo Extended-Botton-Up-Cut-Rod
            if (rbMochilaD.Checked)
            {
                Form2 formfilho = new Form2("Mochila 0-1 (dinâmico)",
@"        private itens Mochila_0_1_Dinâmico(itens[] n, int w, ref lista[] Lista)
        {
            itens resposta;
            resposta.v = 0;
                    ...
                    if (peso >= n[item].p)
                    {
                        ...
                        if (peso >= mochila[peso - n[item].p].p + n[item].p)
                        {
                            ...
                            if (mochila[peso].v < mochila[peso - n[item].p].v + n[item].v)
                            {
                                //Busca em profundidade nos itens da subsolução ótima
                                bool ok = true;
                                int busca = peso - n[item].p;   //Peso anterior
                                while(busca > 0)
                                {
                                    int itemAnt = Lista[busca].i;
                                    //Se o item anterior é igual ao item atual
